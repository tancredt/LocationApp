...
const fetchData = async () => {
  try {
    // Fetch cylinders
    const cylindersResult = await get('/api/inventory/cylinders/');
    if (cylindersResult.ok) {
      cylinders.value = cylindersResult.data;
    }
...
const changeCylinderLocation = async () => {
  ...
  try {
    ...
    console.error('Error changing cylinder location:', error);
    alert('Error changing cylinder location: ' + error.message);
  } finally {
